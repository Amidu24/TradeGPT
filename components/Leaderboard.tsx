...
function getPnLForPeriod(entry: LeaderboardEntry, period: Period): number {
  if (period === "weekly")  return entry.weeklyPnL;
  if (period === "monthly") return entry.monthlyPnL;
...
function RankBadge({ entry }: { entry: LeaderboardEntry }) {
  const score = calculateScore(entry.totalPnL, entry.winRate);
...
function SummaryCard({
  userId, username, entries, period,
}: {
  userId: string;
  username: string;
  ...
  const rank  = calculateRank(score);
  const pnl   = stats ? getPnLForPeriod(stats, period) : 0;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 scrollbar-none">
        ...
        {/* ── Column headers ── */}
        <div className="grid grid-cols-[2rem_1fr_auto_auto_auto] gap-x-3 px-3 mb-2">
          <span />
          ...
          <span className="text-[10px] text-gray-600 uppercase tracking-widest text-right">Streak</span>
        </div>
        ...
        <div className="space-y-1.5">
          {entries.map((entry, i) => {
            ...
            const pnl    = getPnLForPeriod(entry, period);
            ...
            return (
              <div
                key={entry.userId}
                className={`grid grid-cols-[2rem_1fr_auto_auto_auto] gap-x-3 items-center border rounded-xl px-3 py-2.5 transition-all ${rowBg}`}
              ...
              >
                ...
                {/* Streak */}
                <span className="text-xs text-right text-orange-400 font-bold">
